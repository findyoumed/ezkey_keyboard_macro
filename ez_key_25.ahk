...
$SC138::                    ; I want 왼쪽 알트키 key to be "Absolutely Hangul(Korean)" mode key. I hate toggling :(
;;LedLight()
    ret := IME_CHECK("A")
    if %ret% = 0                ; 0 means IME is in English mode now.
    {
...
$SC11D::                    ; I want 오른쪽 컨트롤 key to be "Absolutely English" mode key. I hate toggling :(
;;LedLight()
    ret := IME_CHECK("A")
    if %ret% <> 0               ; 1 means IME is in Hangul(Korean) mode now.
    {
